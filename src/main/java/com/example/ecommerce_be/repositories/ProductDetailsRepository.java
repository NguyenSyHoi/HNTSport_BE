...
public interface ProductDetailsRepository extends JpaRepository<ProductDetails, Long> {
    @Query(value="SELECT p.* from product_details p ",nativeQuery = true)
    List<ProductDetails> getAllProductDetails();


    @Query(value = "SELECT p.* from product_details p where p.product_code =?1 and p.size = ?2",nativeQuery = true)
